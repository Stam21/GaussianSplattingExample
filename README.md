...
For this example we took a 2.5 minutes length video of the room and then converted its frames into a sequence of images. 

In order to train the model we used google collab, and so we share the whole notebook written in collab in case you want to try it.

| Colab | Info
| --- | --- |
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/19qzISTThWFGd3e00rAL6DC0Zqvlt9bT8?usp=sharing) | 3D gaussian splatting room example

The first step is to clone gaussian splatting and install the needed dependencies, as well as colmap and its needed dependencies.
In order to continue with the training of the model, we then need the data from the point cloud.

More specifically, the point cloud was created with the colmap application. The sparse folder and the database file have been stored under the /data/distorted folder and the images have been stored under the /data/images folder. The next step is to convert the distorted images to undistorted before training the model. 

```
python convert.py -s data --skip_matching
```

When the distorted data have been successfully converted, we are ready to train the model.

```
python train.py -s /content/drive/MyDrive/gaussian-splatting/data
```

Finally, we can render the result of the gaussian splatting by running the following commands: 

```
python render.py -m /content/drive/MyDrive/gaussian-splatting/output/df554649-a
ffmpeg -framerate 3 -i /content/drive/MyDrive/gaussian-splatting/output/df554649-a/train/ours_30000/renders/%05d.png -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" -c:v libx264 -r 3 -pix_fmt yuv420p /content/renders.mp4
ffmpeg -framerate 3 -i /content/drive/MyDrive/gaussian-splatting/output/df554649-a/train/ours_30000//gt/%05d.png -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" -c:v libx264 -r 3 -pix_fmt yuv420p /content/gt.mp4 -y
```

Keep in mind that the name of the output model will be different so you have to change "df554649-a" to whatever name your output has.
